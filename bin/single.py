...
os.system('../software/bowtie2 -p 20 -t -x %s/hg19 -U %s/%s -S %s/%s.sam' % (ref,path,fq,path,prefix))
...
os.system('/bin/bioexe/samtools mpileup -B -f %s/hg19.fa %s/%s.rmdup.bam | java -jar ../software/VarScan.v2.3.6.jar mpileup2snp > %s/varscan_%s' % (ref,path,prefix,path,prefix))
...
for line in fu:
        ...
        if username == prefix:
                ...
                os.system('email %s done %s' % (emailaddress,prefix))
fu.close()
